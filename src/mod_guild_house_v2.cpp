...
public:
    ...
    bool RemoveGuildHouse(Player* player)
    {
        ...
        // Lets find all of the gameobjects to be removed       
        GameobjResult = WorldDatabase.PQuery("SELECT `guid`, `id` FROM `gameobject` WHERE `map` = 1 AND `phaseMask` = '%u'", guildPhase);
        // Lets find all of the creatures to be removed
        CreatureResult = WorldDatabase.PQuery("SELECT `guid`, `id` FROM `creature` WHERE `map` = 1 AND `phaseMask` = '%u'", guildPhase);
        ...
        if (CreatureResult) {
            do
            {
                ...
                if (CreatureData const* cr_data = sObjectMgr->GetCreatureData(lowguid)) {
                    if (Creature* creature = ObjectAccessor::GetCreature(*player, ObjectGuid::Create<HighGuid::Unit>(id, lowguid)))
                    {
                        creature->CombatStop();
                        creature->DeleteFromDB();
                        creature->AddObjectToRemoveList();
                        ...
                        sLog->outBasic("GUILDHOUSE: No creature object found %u %u %u", cr_data->id, lowguid, id);
                    }
                } else {
    ...
    void SpawnAssistantNPC(Player* player)
    {
        ...
        if (!creature->Create(sMapMgr->FindMap(1, 0)->GenerateLowGuid<HighGuid::Unit>(), sMapMgr->FindMap(1, 0), GetGuildPhase(player), entry, 0, posX, posY, posZ, ori))
        {
            delete creature;
            return;
        }
...
public:
    ...
    static bool HandleSpawnNPCCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (!creature->Create(player->GetMap()->GenerateLowGuid<HighGuid::Unit>(), player->GetMap(), GetGuildPhase(player), 70102, 0, posX, posY, posZ, ori))
        {
            handler->SendSysMessage("You already have the Guild House Assistant!");
            handler->SetSentErrorMessage(true);
            delete creature;
